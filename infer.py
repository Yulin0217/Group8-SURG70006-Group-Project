...
def vis_pose(img_idx,pose_pred, gt_pose, K, im, require_gt=False):
    length = 15
    short = 4
    thickness = 2
    ...
    #cv2.waitKey(10)
    cv2.imwrite('/workspace/holohub/OccPose/data/infer/{}.png'.format(img_idx),im)
...
def run_inference_folder():
    ...
    try:
        ...
        cfg.model_dir = '/workspace/holohub/OccPose/data/ext_model'
        load_network(network, cfg.model_dir,resume=cfg.resume, epoch=99)
        network.eval()
        ...
        infer_imgs = natsorted(glob.glob(os.path.join(img_folder,'inpainting',cfg.img_format)))

        for rgb_path in infer_imgs:
            print(rgb_path)
            idx = rgb_path.split('/')[-1][:-4]
            print(idx)
            ...
            if debug:
                ...
                record_data = {}
                record_data['kpt_2d']=kpt_2d
                record_data['vertex']=vertex
                record_data['mask']=mask
                record_data['seg']=seg
                np.save('/workspace/holohub/OccPose/data/infer/{}.npy'.format(idx),record_data)
            ...
            im = cv2.imread(rgb_path)
            vis_pose(img_idx,pose_pred, gt_pose, K, im, require_gt=False)
            img_idx+=1
...
def run_cam():
    ...
    try:
        while True:
            # get frame
            ret,frame = cam.read()
	    # convert to RGB (only for openCV)
