...
def Charuco_detect(img, dictionary, camera_matrix, dist_coefs,board, show_axis=False):
	...
	if ids is not None and len(ids) > 0:
		...
		if charucoIds is not None and len(charucoIds) > 0:
			### Draw the Charuco corner
			# cv2.aruco.drawDetectedCornersCharuco(img, charucoCorners, charucoIds, [0, 0, 255])

			retval, rvec, tvec = cv2.aruco.estimatePoseCharucoBoard(charucoCorners, charucoIds, board, camera_matrix,
																dist_coefs, rvec, tvec)
			if retval and show_axis:
				cv2.aruco.drawAxis(img, camera_matrix, dist_coefs, rvec, tvec, 1)
	return img, rvec, tvec

def translate_pts(pts, x_angle, y_angle, z_angle, x_trans, y_trans, z_trans):
	rot2 = angle_to_rot(x_angle, y_angle, z_angle)
	trans_pts = np.array([x_trans, y_trans, z_trans])
	pts = np.dot(rot2,pts.T).T+trans_pts  
	return pts